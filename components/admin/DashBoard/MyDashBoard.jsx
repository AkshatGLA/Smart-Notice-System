...
const fetchData = async () => {
  try {
    ...
    const noticesRes = await fetchWithAuth(`http://localhost:5001/api/notices/created-by/${user.id}`);
    console.log('Notices response status:', noticesRes.status); // Debug log
    
    const usersRes = await fetchWithAuth('http://localhost:5001/api/users/count')
      .catch(() => ({ json: () => ({ count: 0 }) }));
    ...
    console.log('Total reads calculated:', totalReads); // Debug log

    const noticesWithReadCount = noticesData.map(notice => ({
      ...notice,
      readCount: notice.recipient_emails?.length || 0
    }));
    console.log('Processed notices:', noticesWithReadCount); // Debug log
    ...
    console.error('Full error details:', err); // More detailed error log
    setError(err.message);
